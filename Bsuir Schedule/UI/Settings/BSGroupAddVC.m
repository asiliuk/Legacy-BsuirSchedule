...
- (void)dismissWithChanges:(BOOL)changes {
    [self dismissWithChanges:changes complition:^{
        [self.delegate groupAddingScreen:self dismissWithGroupNumber:self.groupNumberTF.text andSubgroup:self.subgroupNumberTF.text];
    }];
}
- (void)dismissWithChanges:(BOOL)changes complition:(void(^)(void))complition {
...
- (IBAction)okPressed:(id)sender {
    if ([self.groupNumberTF.text isEqual:@""] || [self.subgroupNumberTF.text isEqual:@""]) {
        ...
        __weak typeof(self) weakSelf = self;
        [self dismissWithChanges:YES complition:^{
            [weakSelf.delegate groupAddingScreen:self dismissWithGroupNumber:self.groupNumberTF.text andSubgroup:self.subgroupNumberTF.text];
        }];
    }
}
